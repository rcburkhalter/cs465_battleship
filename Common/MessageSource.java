...
/**
 * This class represents an abstract message source &ndash; a subject in the
 * observer pattern.
 * This class should be extended by classes that want to deliver messages to
 * some set of interested
 * parties. These &quot;interested parties&quot; are decoupled from the
 * implementation of this
 * class &ndash; those classes must implement the <code>MessageListener</code>
 * interface.
 *
 * <b> The two concepts have been decoupled from one another:
 * Meaning that classes that are the source of message must implement the
...
public abstract class MessageSource {
    ...
    protected void closeMessageSource() {
        /*
         * Here we need to iterate over a *copy* of our messageListeners list. The
         * reason is
         * because if the listener's 'sourceClosed' method removes that listener from
         * this subject,
         * we'd get a ConcurrentModificationException if we were iterating over the
         * original list.
         */
        for (MessageListener listener : new ArrayList<MessageListener>(messageListeners)) {
            try {
                ...
                /*
                 * We're doing this on a best-effort basis. If something goes wrong, we don't
                 * want
    ...
    protected void notifyReceipt(String message) {
        for (MessageListener listener : new ArrayList<MessageListener>(messageListeners)) {
            /*
             * We wrap this in a try/catch block so that just in case one of our observers
             * screws
